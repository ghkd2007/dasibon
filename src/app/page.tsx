...
const defaultSections: WorshipSection[] = [
  ...
  },
  {
    ...
    label: "나눔 질문",
    title: "[나눔 질문]",
    body: <p className="text-[15px] leading-6">나눔 질문을 입력해 주세요.</p>,
  },
  {
    id: "announcements",
    label: "광고",
    ...
    <ul className="space-y-3">
      {praiseCards.map((card, i) => (
        <li key={i}>
          {card.imageUrl ? (
            <a
              href={`/score?url=${encodeURIComponent(card.imageUrl)}`}
              className="block rounded-xl border border-[#e5d6c0] bg-[#fbf5eb]/95 p-4 text-[15px] font-medium text-foreground shadow-sm hover:bg-[#f5ebe0] active:scale-[0.99] transition"
            >
              <span className="block">{card.title || "찬양"}</span>
            </a>
          ) : (
            <div className="rounded-xl border border-[#e5d6c0] bg-[#fbf5eb]/95 p-4 text-[15px] text-foreground">
              {card.title || "—"}
            </div>
          )}
        </li>
    ...
    { id: "passage", label: "말씀", title: "[말씀]", body: html(b.passage || "—") },
    {
      ...
      body: (
        <div className="space-y-2 text-[15px] leading-6">
          {b.sermonTitleMain && <p className="font-medium" style={b.sermonTitleMainColor ? { color: b.sermonTitleMainColor } : undefined}>{b.sermonTitleMain}</p>}
          {b.sermonTitleSub && <p className="text-foreground/80" style={b.sermonTitleSubColor ? { color: b.sermonTitleSubColor } : undefined}>{b.sermonTitleSub}</p>}
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="h-8 w-8 flex items-center justify-center rounded-full border border-[#d3c2aa] bg-white/70 text-foreground/80 hover:bg-white/90"
                aria-label="홈(인트로)"
                title="인트로 화면으로"
              >
              ...
              <button
                ...
                className="hidden h-8 w-8 flex items-center justify-center rounded-full border border-[#d3c2aa] bg-white/70 text-[11px] text-foreground/80 hover:bg-white/90"
                aria-label="글자 작게"
              >
                A-
              </button>
              <button
                ...
                className="hidden h-8 w-8 flex items-center justify-center rounded-full border border-[#d3c2aa] bg-white/80 text-[11px] font-semibold text-foreground/80 hover:bg-white/90"
                aria-label="글자 크게"
              >
                A+
              </button>
            ...
            {sections.map((section, index) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 px-2.5 py-1.5 rounded-t border-b-2 text-[12px] transition-colors ${
                    isActive
                      ? "border-[#3b2a20] text-foreground font-semibold"
                      : "border-transparent text-foreground/70 hover:text-foreground/85"
                  }`}
                >
                  {section.label}
                </button>
